...
def _newest_xlsx_since(folder: Path, since_ts: float) -> Optional[Path]:
    ...
    timestamped fallback a checker writes when its default file is locked)."""
...
def run_checker(checker: Checker, items: list, *, aux_lists: Optional[dict] = None,
    ...
    *items* is a list of GitHub-link paper objects.  We inject them via a temp
    file + env var (``PAPERS_JSON``) and run the checker as a subprocess,
    inheriting the repo's ``.env`` (the checker calls ``load_dotenv()`` itself,
    so GitHub/LLM keys are picked up).

    *on_line(line)*, if given, is called for each stdout line as it arrives (so
    the UI can show live progress); otherwise output is just captured.
    """
    started = time.time()
    with tempfile.TemporaryDirectory(prefix="checker_ui_") as tmp:
        tmp = Path(tmp)
        env = os.environ.copy()
        ...
        # hides all progress until the process exits.
...
def build_input_quality_xlsx(items: list, flagged: list) -> bytes:
    ...
    *flagged* is the list of ``(index, paper, warnings)`` from
    ...
    headers = ["#", "Title", "Repo", "Data-quality warnings"]
    widths = [5, 55, 50, 75]
    ...
    for i, paper in enumerate(items, 1):
        ...
        wcell = ws.cell(row=i + 1, column=4, value="OK" if not warns else "\n".join(warns))
